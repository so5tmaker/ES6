...
Созданный поисковый индекс (хеш-таблица) позволяет быстро находить нужные вхождения количества уникальных слов 
в каждый документ. Затем с помощью этого индекса корректно вычисляется релевантность каждого документа для 
данного запроса, учитывая количество вхождений слов в каждый документ, даже если слово встречается 
несколько раз в одном документе.

– ВРЕМЕННАЯ СЛОЖНОСТЬ –
Построение индекса имеет временную сложность O(nm), где n - количество документов, 
а m - средняя длина документа. Получение релевантных документов имеет временную сложность O(ql), 
где q - количество уникальных слов в запросе, а l - среднее количество вхождений слова в документ. 
Сортировка релевантных документов имеет временную сложность O(dlog(d)), где d - количество документов. 
Итоговая временная сложность поиска для k запросов составляет O(k(nm + ql + d*log(d))).

– ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ –
Для построения индекса используется дополнительная память O(n*m), где n - количество документов, m - средняя длина документа. 
...
Общая пространственная сложность программы может быть оценена как O(n*m + q + d) или O(n*m).
*/
